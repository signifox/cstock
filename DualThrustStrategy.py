...
# @Desc    : DualThrust Strategy with Long and Short Operations
...
class DualThrustStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        size1 = int((cash * self.params.risk_per_trade) / (self.atr[0] * 2))
        ...
        if self.position:
            if self.position.size > 0:  # 多头
                ...
                trailing_stop = self.highest_price - self.atr[0] * 2
                ...
                trailing_stop = self.lowest_price + self.atr[0] * 2
                if current_price >= stop_loss:
                    self.order = self.close(data=self.minute_data)
            ...
            if (
                ...
                short_size = int(size * 0.5)
                if short_size < 1:
                    short_size = 1
                self.order = self.sell(data=self.minute_data, size=short_size)
                self.log(
                    f"卖出: {current_price}, 下轨: {self.sell_line}, 数量: {short_size}"
                )
                self.lowest_price = current_price
